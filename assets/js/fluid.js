 ...
 * Configured to match DagsHub.com's exact parameters
 */
(function () {
  'use strict';
  ...
  if (/Mobi|Android/i.test(navigator.userAgent) && window.innerWidth < 769) return;
  ...
  function drawDisplay(target) {
    ...
    if (config.BLOOM) {
      ...
      gl.uniform1i(displayMaterial.uniforms.uDithering, ditheringTextureObj.attach(2));
      var scale = getTextureScale(ditheringTextureObj, width, height);
  ...
  // ── Event listeners ──
  // Use document-level listeners since canvas has pointer-events: none
  ...
  document.addEventListener('touchstart', function (e) {
    ...
    for (var i = 0; i < touches.length; i++) {
      updatePointerDownData(
        ...
        scaleByPixelRatio(touches[i].pageX),
        scaleByPixelRatio(touches[i].pageY)
      );
    }
  });
